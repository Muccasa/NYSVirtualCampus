...
import { Plus, Users, BookOpen, FileText, Trash2, Edit, Upload, FileIcon } from "lucide-react";
import { coursesApi, assignmentsApi, uploadsApi, announcementsApi, usersApi } from "@/lib/api";
...
export default function TutorDashboard({ onNavigate, onOpenCourse, onOpenAssignment, onAddAssignment, onUpdateAssignment, onDeleteAssignment, assignmentsFromApp, onAssignmentsChange, initialTab, showHeader = true, hideAssignments = false, hideStudents = false, hideCourses = false, hideContent = false, hideAnnouncements = false }: TutorDashboardProps) {
  ...
  const [assignments, setAssignments] = useState<AssignmentDraft[]>([
    {
      ...
      type: "upload",
      dueDate: "2024-02-15",
      instructions: "Create your first program",
      questions: [{ text: "Create a simple calculator" }],
    },
  ]);
  ...
  const [announcementFilterCourse, setAnnouncementFilterCourse] = useState<string | undefined>(undefined);
  const [isCreateAnnouncementOpen, setIsCreateAnnouncementOpen] = useState(false);
  const [newAnnouncement, setNewAnnouncement] = useState({ title: '', message: '', courseId: '' , isGlobal: false});

  const [newModule, setNewModule] = useState({ courseId: "", title: "", chapterTitle: "", notes: "", files: [] as File[], pptLink: "" });
  ...
  const [bulkTransfer, setBulkTransfer] = useState({ fromCourse: "", toCourse: "", students: [] as string[] });
  ...
  const handleCreateModule = async () => {
    ...
    try {
      ...
      setNewModule({ courseId: "", title: "", chapterTitle: "", notes: "", files: [], pptLink: "" });
      setIsCreateModuleOpen(false);
      toast({ title: 'Module created', description: 'Module and chapter created successfully.' });
    } catch (err) {
      console.error('Create module failed', err);
      toast({ title: 'Create failed', description: err instanceof Error ? err.message : 'Failed to create module', variant: 'destructive' });
    }
  };
  ...
  const handleEnrollStudent = () => {
    ...
    const newStudent = { id: Date.now().toString(), name: enrollStudent.name, email: enrollStudent.email, courseId: enrollStudent.courseId };
  ...
  const handleBulkTransfer = async () => {
    if (!bulkTransfer.fromCourse || !bulkTransfer.toCourse || bulkTransfer.students.length === 0) {
      toast({ title: 'Invalid request', description: 'Select source, destination and at least one student', variant: 'destructive' });
      return;
    }

    const fromCourseObj = allCourses.find((c) => c.id === bulkTransfer.fromCourse);
    const toCourseObj = allCourses.find((c) => c.id === bulkTransfer.toCourse);
  ...
  const doConfirmTransfer = async () => {
    ...
    try {
      const res: any = await coursesApi.bulkTransfer({ fromCourseId: bulkTransfer.fromCourse, toCourseId: bulkTransfer.toCourse, studentIds: bulkTransfer.students });
      // optimistic local update: set each transferred student's courseId to the destination so UI reflects change
      setStudents((prev) => prev.map((s) => (bulkTransfer.students.includes(s.id) ? { ...s, courseId: bulkTransfer.toCourse } : s)));
      toast({ title: 'Transfer complete', description: `Transferred ${res.results?.transferred?.length || bulkTransfer.students.length} students.` });
      setIsBulkTransferOpen(false);
      setBulkTransfer({ fromCourse: "", toCourse: "", students: [] });
    } catch (err) {
      console.error('Bulk transfer failed', err);
      toast({ title: 'Transfer failed', description: err instanceof Error ? err.message : 'Bulk transfer failed', variant: 'destructive' });
    } finally {
  ...
  const handleRemoveStudent = (studentId: string, courseId: string) => {
    if (confirm("Remove this student from the course?")) {
      setStudents(students.filter((s) => !(s.id === studentId && s.courseId === courseId)));
    }
  };
  ...
  const handleRemoveAssignment = (assignmentId?: string) => {
    ...
    (async () => {
      try {
        ...
        setAssignments((prev) => prev.filter((a) => ((a as any).id !== assignmentId && (a as any)._id !== assignmentId)));
        if (onDeleteAssignment) onDeleteAssignment(assignmentId);
        toast({ title: 'Assignment deleted', description: 'Assignment was removed.' });
      } catch (err) {
  ...
  const handleDeleteCourse = (courseId?: string) => {
    if (!courseId) return;
    if (!confirm('Delete this course? This will remove enrollments and assignments.')) return;
    (async () => {
      try {
        ...
        toast({ title: 'Delete failed', description: err instanceof Error ? err.message : 'Failed to delete course', variant: 'destructive' });
      }
    })();
  };
  ...
  const handleDeleteAnnouncement = (announcementId?: string) => {
    ...
    (async () => {
      try {
        ...
        toast({ title: 'Announcement deleted', description: 'Announcement removed.' });
      } catch (err) {
        toast({ title: 'Delete failed', description: err instanceof Error ? err.message : 'Failed to delete announcement', variant: 'destructive' });
      }
    })();
  };
  ...
  const handleEditCourseSave = async () => {
    ...
    try {
      ...
      setCourses((prev) => prev.map((c) => (c.id === editCourseDraft.id ? { ...c, ...editCourseDraft } : c)));
      setIsEditCourseOpen(false);
      setEditCourseDraft(null);
      toast({ title: 'Course updated', description: 'Course details were updated.' });
    } catch (e) {
      toast({ title: 'Update failed', description: e instanceof Error ? e.message : 'Failed to update', variant: 'destructive' });
    }
  };
  ...
  const handleEditAssignmentSave = async () => {
    ...
    try {
      ...
      setAssignments((prev) => prev.map((a) => (a.id === editAssignmentDraft.id ? { ...a, ...editAssignmentDraft } : a)));
  ...
  const defaultTab = initialTab || (hideCourses ? (hideAssignments ? (hideStudents ? (hideContent ? (hideAnnouncements ? "" : "announcements") : "content") : "students") : "assignments") : "courses");
      ...
      <div className="flex justify-between items-center">
        {showHeader ? (
          <div>
            ...
            <p className="text-muted-foreground">Manage courses, students, content, and assignments</p>
          </div>
        ) : (
          <div />
        )}
  ...
  <Tabs defaultValue={defaultTab} className="space-y-4">
      <TabsList>
          {!hideCourses && <TabsTrigger value="courses">My Courses</TabsTrigger>}
          {!hideAssignments && <TabsTrigger value="assignments">Assignments</TabsTrigger>}
          {!hideStudents && <TabsTrigger value="students">Students</TabsTrigger>}
          {!hideContent && <TabsTrigger value="content">Course Content</TabsTrigger>}
          {!hideAnnouncements && <TabsTrigger value="announcements">Announcements</TabsTrigger>}
        </TabsList>
            ...
            <Card key={course.id}>
              <CardContent className="p-6">
                <div className="flex justify-between items-start mb-4">
                  <div>
                    <h3 className="text-xl font-semibold mb-2"><button className="text-left hover:underline" onClick={() => { if (onOpenCourse) onOpenCourse(course.id); }}>{course.title}</button></h3>
            ...
            <DialogContent>
              ...
              <CreateAssignment
                ...
                onCreated={(assignment) => {
                  ...
                  if (onAddAssignment) onAddAssignment(assignment);
  ...
  <TabsContent value="students" className="space-y-4">
              ...
              <DialogContent>
                ...
                <div className="space-y-4">
                  <div>
                    ...
                    <Select value={enrollStudent.courseId} onValueChange={(value) => setEnrollStudent({ ...enrollStudent, courseId: value })}>
                      <SelectTrigger>
                  ...
                  <div>
                    ...
                    <Input value={enrollStudent.name} onChange={(e) => setEnrollStudent({ ...enrollStudent, name: e.target.value })} placeholder="Full Name" />
                  </div>
                  <div>
                    <Label>Student Email</Label>
                    <Input type="email" value={enrollStudent.email} onChange={(e) => setEnrollStudent({ ...enrollStudent, email: e.target.value })} placeholder="student@example.com" />
                  </div>
                  <Button onClick={handleEnrollStudent} className="w-full">Enroll Student</Button>
                </div>
              </DialogContent>
            </Dialog>
                ...
                <div className="space-y-4">
                  <div className="space-y-1">
                    ...
                    <div className="flex gap-2">
                      <Input className="flex-1" placeholder="Search courses..." value={bulkSearch} onChange={(e) => setBulkSearch(e.target.value)} />
                      <Select value={bulkTransfer.fromCourse} onValueChange={(value) => {
                        const picked = allCourses.find((c) => (c.id || c._id) === value);
                        setBulkTransfer({ ...bulkTransfer, fromCourse: value, students: [] });
                        ...
                        <SelectContent className="max-h-64 overflow-y-auto">
                          {allCourses.filter((c) => c.title.toLowerCase().includes(bulkSearch.toLowerCase())).map((course) => (
                            <SelectItem key={course.id} value={course.id}>{course.title}</SelectItem>
                          ))}
                        </SelectContent>
                      </Select>
                  ...
                  <div>
                    ...
                    <div className="flex gap-2">
                      <Input className="flex-1" placeholder="Search courses..." value={bulkToSearch} onChange={(e) => setBulkToSearch(e.target.value)} />
                      <Select value={bulkTransfer.toCourse} onValueChange={(value) => {
                        const picked = allCourses.find((c) => (c.id || c._id) === value);
                        setBulkTransfer({ ...bulkTransfer, toCourse: value });
                        ...
                        <SelectContent className="max-h-64 overflow-y-auto">
                          {allCourses.filter((c) => c.id !== bulkTransfer.fromCourse && c.title.toLowerCase().includes(bulkToSearch.toLowerCase())).map((course) => (
                            <SelectItem key={course.id} value={course.id}>{course.title}</SelectItem>
                          ))}
                        </SelectContent>
                      </Select>
                    ...
                    <div>
                      ...
                      <div className="border rounded-md p-4 max-h-48 overflow-y-auto space-y-2">
                        ...
                        <label className="flex items-center gap-2">
                          <input type="checkbox" className="rounded" disabled={getStudentsForCourse(bulkTransfer.fromCourse).length === 0} checked={
                            // checked when every visible student id is present in bulkTransfer.students
                            (() => {
                              const visible = getStudentsForCourse(bulkTransfer.fromCourse).map((s) => s.id);
                              return visible.length > 0 && visible.every((id) => bulkTransfer.students.includes(id));
                            })()
                          } onChange={(e) => {
                            const visible = getStudentsForCourse(bulkTransfer.fromCourse).map((s) => s.id);
                            if (e.target.checked) {
                              // add all visible ids (avoid duplicates)
                              const merged = Array.from(new Set([...bulkTransfer.students, ...visible]));
                              setBulkTransfer({ ...bulkTransfer, students: merged });
                            } else {
                              // remove visible ids from selection
                              setBulkTransfer({ ...bulkTransfer, students: bulkTransfer.students.filter((id) => !visible.includes(id)) });
                            }
                          }} />
                          <span className="font-medium">Select all visible students ({getStudentsForCourse(bulkTransfer.fromCourse).length})</span>
                        </label>

                        {getStudentsForCourse(bulkTransfer.fromCourse).map((student) => (
                          <label key={student.id} className="flex items-center gap-2">
                            <input type="checkbox" checked={bulkTransfer.students.includes(student.id)} onChange={(e) => {
                              if (e.target.checked) setBulkTransfer({ ...bulkTransfer, students: [...bulkTransfer.students, student.id] });
                              else setBulkTransfer({ ...bulkTransfer, students: bulkTransfer.students.filter((id) => id !== student.id) });
                            }} className="rounded" />
                  ...
                  <Button onClick={handleBulkTransfer} disabled={!bulkTransfer.fromCourse || !bulkTransfer.toCourse || bulkTransfer.students.length === 0 || isTransferring} className="w-full">
                    {isTransferring ? 'Transferring...' : `Transfer ${bulkTransfer.students.length} Student(s)`}
                  </Button>
                </div>
              </DialogContent>
            </Dialog>
              ...
              <DialogContent>
                ...
                <div className="space-y-4">
                  ...
                  <div className="flex gap-2 justify-end">
                    <Button variant="outline" onClick={() => setIsConfirmBulkOpen(false)} disabled={isTransferring}>Cancel</Button>
                    <Button onClick={doConfirmTransfer} disabled={isTransferring}>{isTransferring ? 'Transferring...' : 'Confirm transfer'}</Button>
                  </div>
                </div>
              </DialogContent>
            </Dialog>
          ...
          {courses.map((course) => {
            ...
            return (
              <Card key={course.id}>
                <CardHeader>
                  <CardTitle className="text-lg">{course.title} - Students ({courseStudents.length})</CardTitle>
                </CardHeader>
                <CardContent>
                    ...
                    <div className="space-y-2">
                      {courseStudents.map((student) => (
                        <div key={student.id} className="flex items-center justify-between p-3 border rounded-lg">
                          ...
                          <Button variant="ghost" size="icon" onClick={() => handleRemoveStudent(student.id, course.id)}>
                            <Trash2 className="h-4 w-4 text-destructive" />
                          </Button>
                        </div>
                      ))}
          ...
          <Dialog open={isCreateModuleOpen} onOpenChange={setIsCreateModuleOpen}>
            ...
            <DialogContent className="max-w-2xl">
              ...
              <div className="space-y-4">
                <div>
                  <Label>Select Course *</Label>
                  <Select value={newModule.courseId} onValueChange={(value) => setNewModule({ ...newModule, courseId: value })}>
                    <SelectTrigger>
                ...
                <div>
                  ...
                  <Input value={newModule.title} onChange={(e) => setNewModule({ ...newModule, title: e.target.value })} placeholder="e.g., Module 1: Introduction" />
                </div>

                <div>
                  <Label>Chapter Title</Label>
                  <Input value={newModule.chapterTitle} onChange={(e) => setNewModule({ ...newModule, chapterTitle: e.target.value })} placeholder="e.g., Chapter 1: Getting Started" />
                </div>

                <div>
                  <Label>Chapter Notes</Label>
                  <Textarea value={newModule.notes} onChange={(e) => setNewModule({ ...newModule, notes: e.target.value })} placeholder="Rich text notes for this chapter" rows={5} />
                </div>
                ...
                <div>
                  ...
                  <input
                    type="file"
                    multiple
                    onChange={(e) => setNewModule({ ...newModule, files: e.target.files ? Array.from(e.target.files) : [] })}
                    className="mt-1"
                  />
                  {newModule.files && newModule.files.length > 0 && (
                    <p className="text-sm text-muted-foreground mt-2">{newModule.files.length} file(s) selected</p>
                  )}
                </div>

                <div>
                  <Label>PPT Link (optional)</Label>
                  <Input value={newModule.pptLink} onChange={(e) => setNewModule({ ...newModule, pptLink: e.target.value })} placeholder="https://example.com/slide.pptx" />
                  <p className="text-xs text-muted-foreground mt-1">PPT links are stored at the chapter level. If you want a course-level PPT, use the Course editor.</p>
                </div>

                <Button onClick={handleCreateModule} className="w-full">Create Module & Chapter</Button>
              </div>
            </DialogContent>
          </Dialog>
          ...
          <div className="space-y-4">
            {courses.map((course) => {
              ...
              return (
                <Card key={course.id}>
                  <CardHeader>
                    <CardTitle className="text-lg">{course.title} - Content Modules</CardTitle>
                  </CardHeader>
                  <CardContent>
                          ...
                          <Card key={module.id} className="p-4">
                            ...
                            <div className="space-y-2">
                              {module.chapters.map((chapter) => (
                                <div key={chapter.id} className="p-3 bg-muted rounded">
                                  <p className="font-medium text-sm">{chapter.title}</p>
                                  <p className="text-xs text-muted-foreground mt-1">{chapter.notes}</p>
                                  {chapter.files && chapter.files.length > 0 && (
                                    <p className="text-xs text-muted-foreground mt-2">{chapter.files.length} files attached</p>
                                  )}
                                </div>
                              ))}
                            </div>
        ...
        <TabsContent value="announcements" className="space-y-4">
          <div className="flex justify-between items-center">
              ...
              <Select value={announcementFilterCourse ?? '__all__'} onValueChange={(v) => { const courseId = v === '__all__' ? undefined : v; setAnnouncementFilterCourse(courseId); fetchAnnouncements(courseId); }}>
                <SelectTrigger>
                  <SelectValue placeholder="All courses" />
                </SelectTrigger>
                <SelectContent>
                ...
                <DialogContent className="max-w-lg">
                  ...
                  <div className="space-y-4">
                    <div>
                      ...
                      <Select value={newAnnouncement.courseId || '__global__'} onValueChange={(v) => setNewAnnouncement({ ...newAnnouncement, courseId: v === '__global__' ? '' : v })}>
                        <SelectTrigger>
                    ...
                    <div>
                      <Label>Message</Label>
                      <Textarea value={newAnnouncement.message} onChange={(e) => setNewAnnouncement({ ...newAnnouncement, message: e.target.value })} rows={4} />
                    </div>
                    <div className="flex gap-2 justify-end">
                      <Button variant="outline" onClick={() => { setIsCreateAnnouncementOpen(false); setNewAnnouncement({ title: '', message: '', courseId: '', isGlobal: false }); }}>Cancel</Button>
                      <Button onClick={async () => {
                        try {
                          const payload: any = { title: newAnnouncement.title, message: newAnnouncement.message };
                          if (newAnnouncement.courseId) payload.courseId = newAnnouncement.courseId;
                          else payload.isGlobal = true;
                          const res = await announcementsApi.create(payload);
                          setAnnouncements((prev) => [res, ...prev]);
                          setIsCreateAnnouncementOpen(false);
                          setNewAnnouncement({ title: '', message: '', courseId: '', isGlobal: false });
                          toast({ title: 'Announcement created', description: 'Announcement was posted.' });
                        } catch (err) {
                  ...
                  <CardContent>
                    <div className="flex justify-between items-start">
                      <div>
                        ...
                        <p className="text-xs text-muted-foreground mt-2">{a.courseId ? `Course: ${a.courseId.title || a.courseId}` : 'Global'}</p>
                      </div>
                      <div className="text-xs text-muted-foreground">{new Date(a.createdAt).toLocaleString()}</div>
                      <div className="ml-4 flex items-center gap-2">
                        <Button variant="ghost" size="icon" onClick={() => { if (onNavigate) onNavigate('announcements'); else { try { window.location.hash = 'announcements'; } catch (e) {} } }} title="Open announcements">
                          <BookOpen className="h-4 w-4" />
                        </Button>
                        <Button variant="ghost" size="icon" onClick={() => handleDeleteAnnouncement(a.id || a._id)}>
                          <Trash2 className="h-4 w-4 text-destructive" />
                        </Button>
                      </div>
                    </div>
        ...
        <DialogContent className="max-w-2xl">
          ...
          {editCourseDraft && (
            <CreateCourse
              ...
              onCancel={() => { setIsEditCourseOpen(false); setEditCourseDraft(null); }}
              onUpdated={(updated: any) => {
                setCourses((prev) => prev.map((c) => (c.id === (updated.id || updated._id) ? { ...c, ...updated } : c)));
                setIsEditCourseOpen(false);
                setEditCourseDraft(null);
              }}
            />
        ...
        <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
          ...
          {editAssignmentDraft && (
            <CreateAssignment
              ...
              onUpdated={(updated: any) => {
                setAssignments((prev) => prev.map((a) => (a.id === (updated.id || updated._id) ? { ...a, ...updated } : a)));
                setIsEditAssignmentOpen(false);
                setEditAssignmentDraft(null);
                toast({ title: 'Assignment updated', description: 'Assignment updated successfully.' });
              }}
